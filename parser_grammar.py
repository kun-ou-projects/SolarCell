...
range = (Optional(R(r'^[\-–−]$')) + (and_range | to_range | up_to_range | spaced_range | joined_range)).add_action(merge)
value = (Optional(R(r'^[\-–−]$')) +
         Optional(R(r'^[~∼˜\<\>\≤\≥]$')) +
         Optional(R(r'^[\-\–\–\−±∓⨤⨦±]$')) +
...
e_volt = (exp | power | range | value | ordinal)('value')  # match ev values
